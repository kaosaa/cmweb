# CM (Claude Manager) - Web & 桌面端

> Claude Code CLI 的现代化 Web / 桌面管理界面

## 项目简介

CM (Claude Manager) 是一个为 Claude Code CLI 设计的本地界面，同时支持 **Web 浏览器** 和 **桌面端（Windows / macOS）**。提供直观、现代化的聊天交互体验，用户可以便捷地与 Claude AI 进行交互，管理对话历史，查看工具调用详情等。

桌面端基于 **Tauri v2**（Rust 内核 + 原生 WebView），与 Web 端共享同一份 React 源码，零代码分叉。
...
- **Vite** - 快速的前端构建工具
- **Tauri v2** - 桌面端框架（Rust 内核，原生 WebView）
...
# 桌面端开发（自动启动 Vite + Tauri 窗口，支持热重载）
npm run tauri:dev
```
...
# 桌面端构建（输出安装包到 src-tauri/target/release/bundle/）
npm run tauri:build
```
...
**注意**: 本项目需要配合 Claude Code CLI 后端服务使用。桌面端架构详见 [DESKTOP.md](./DESKTOP.md)。
